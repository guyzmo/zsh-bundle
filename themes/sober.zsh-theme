...
if [[ ${USER} == "root" ]]; then
    if [[ $(uname) == "Darwin" ]]; then
        PROMPT='${CC}[${RC}$(battery_pct_prompt)${CC}]-(${RC}$fg[red]%n${RC}${CCH}@%m${RC}${CC})-(${RC}%D{%H:%M}${CC})${RC} ${ret_prompt} '
    else
        PROMPT='${BATT_PROMPT}${CC}(${RC}$fg[red]%n${RC}${CCH}@%m${RC}${CC})-(${RC}%D{%H:%M}${CC})${RC} ${ret_prompt} '
    fi
else
    if [[ $(uname) == "Darwin" ]]; then
